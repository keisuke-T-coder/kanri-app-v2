...
import { Wrench, Calculator, Clipboard, Check, Search, Trash2, X } from "lucide-react";
...
export function CaseTools() {
  ...
  return (
    <div className="flex flex-col">
      ...
      <main className="p-4 space-y-6 max-w-2xl mx-auto w-full pb-10">
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">品番・機種検索</label>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
            <input type="text" placeholder="例: JA1000, JH9015..." value={input} onChange={(e) => setInput(e.target.value)} className="w-full bg-white border border-black/5 rounded-2xl py-3 pl-10 pr-4 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm" />
          </div>
        </div>
        {mode === "MODEL" && (
          ...
          {selectedParts.length === 0 ? (<div className="flex flex-col items-center justify-center py-10 bg-white/50 border-2 border-dashed border-black/5 rounded-3xl opacity-40"><Calculator className="w-8 h-8 mb-2" /><p className="text-[10px] font-bold uppercase">部品を追加すると見積もりが開始されます</p></div>) : (
            <div className="space-y-3">
              {selectedParts.map((code, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-white rounded-2xl border border-black/5 animate-in zoom-in-95 duration-200">
